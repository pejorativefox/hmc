...
def apply_dotfiles(dotfiles: dict, cfg_dir: Path, hm: Path, dry_run: bool, show_diff: bool, force: bool) -> int:
    ...
    for dest_rel, spec in (dotfiles or {}).items():
        ...
        if has_src:
            ...
            if cur == want:
                if mode:
                    changed += apply_mode(dest, mode, dry_run)
                continue
            changed += 1
            if dry_run:
                print(f"+ write {dest}")
...
def parse_mode(dest_label, mode: str) -> int:
    try:
        return int(mode, 8)
    except (ValueError, TypeError):
        sys.exit(f"{dest_label}: invalid mode {mode!r}, use e.g. \"0o600\"")


def apply_mode(dest: Path, mode: str, dry_run: bool) -> int:
    """Ensure dest has mode; return 1 if changed/would-change, 0 if already correct."""
    m = parse_mode(dest, mode)
    try:
        if (dest.stat().st_mode & 0o777) == (m & 0o777):
            return 0
    except OSError:
        pass
    if dry_run:
...
def _systemctl_ok(cmd: list[str]) -> bool:
    try:
...
def apply_services(services: dict, hm: Path, dry_run: bool, show_diff: bool) -> int:
    ...
    if systemctl:
        to_enable = [n for n in wants_enable
                     if not _systemctl_ok(["systemctl", "--user", "is-enabled", n])]
        to_start = [n for n in wants_start
                    if not _systemctl_ok(["systemctl", "--user", "is-active", n])]
    else:
        to_enable, to_start = wants_enable, wants_start
    if dry_run:
        if changed or to_enable or to_start:
            if changed:
        ...
        return changed + len(to_enable) + len(to_start)
    if not systemctl:
...
def _expand_home_entry(entry: str, hm: Path) -> str:
    if entry == "~":
        return str(hm)
    if entry.startswith("~/"):
        return str(hm / entry[2:])
    return entry


def _expand_path_value(value, hm: Path) -> str:
    items = value if isinstance(value, list) else str(value).split(":")
    return ":".join(_expand_home_entry(str(p), hm) for p in items if str(p) != "")
...
def apply_env(env: dict, hm: Path, dry_run: bool) -> int:
    ...
    for k, v in env.items():
        if k in ("PATH_prepend", "PATH_append"):
            expanded = _expand_path_value(v, hm)
            quoted = _quote_dq(expanded)
            if k == "PATH_prepend":
                lines.append(f'export PATH="{quoted}:$PATH"')
            else:
                lines.append(f'export PATH="$PATH:{quoted}"')
        else:
    ...
    for dest_rel, spec in (cfg.get("dotfiles") or {}).items():
        ...
        if "source" in spec:
            if not isinstance(spec["source"], str):
                sys.exit(f"check: dotfiles[{dest_rel!r}] 'source' must be a string")
            if not (cfg_dir / spec["source"]).exists():
                sys.exit(f"check: dotfiles[{dest_rel!r}] source missing: {cfg_dir / spec['source']}")
        if "text" in spec and not isinstance(spec["text"], str):
            sys.exit(f"check: dotfiles[{dest_rel!r}] 'text' must be a string")
        if "mode" in spec:
            if not isinstance(spec["mode"], str):
                sys.exit(f"check: dotfiles[{dest_rel!r}] invalid mode {spec['mode']!r}, use e.g. \"0o600\"")
            try:
                int(spec["mode"], 8)
            except ValueError:
                sys.exit(f"check: dotfiles[{dest_rel!r}] invalid mode {spec['mode']!r}, use e.g. \"0o600\"")
        dest = (hm / dest_rel).expanduser()
        if not in_home(dest, hm):
            sys.exit(f"check: dotfiles[{dest_rel!r}] resolves outside $HOME, switch needs --force")
    # validate packages
    pkgs = cfg.get("packages") or {}
    if not isinstance(pkgs, dict):
        sys.exit("check: [packages] must be a table of backend = [pkgs]")
    unknown = set(pkgs) - set(BACKENDS)
    if unknown:
        sys.exit(f"check: unknown backends {sorted(unknown)}")
    for backend, lst in pkgs.items():
        if not isinstance(lst, list) or not all(isinstance(p, str) for p in lst):
            sys.exit(f"check: packages[{backend!r}] must be a list of strings")
    for name, spec in (cfg.get("services") or {}).items():
        if not isinstance(spec, dict) or "text" not in spec:
            sys.exit(f"check: services[{name!r}] needs text")
        if not isinstance(spec["text"], str):
            sys.exit(f"check: services[{name!r}] 'text' must be a string")
        for flag in ("enable", "start"):
            if flag in spec and not isinstance(spec[flag], bool):
                sys.exit(f"check: services[{name!r}] {flag!r} must be true/false")
    hooks = cfg.get("hooks") or {}
    if not isinstance(hooks, dict) or set(hooks) - {"pre", "post"}:
        sys.exit(f"check: hooks only supports pre/post, got {sorted(hooks) if isinstance(hooks, dict) else hooks!r}")
    for phase in ("pre", "post"):
        cmds = hooks.get(phase) or []
        if not isinstance(cmds, list) or not all(isinstance(c, str) for c in cmds):
            sys.exit(f"check: hooks.{phase} must be a list of strings")
    env = cfg.get("env") or {}
    if not isinstance(env, dict):
        sys.exit("check: [env] must be a table")
    for k, v in env.items():
        if k in ("PATH_prepend", "PATH_append"):
            if not isinstance(v, (str, list)) or (isinstance(v, list) and not all(isinstance(p, str) for p in v)):
                sys.exit(f"check: env.{k} must be a string or list of strings")
        elif not isinstance(v, (str, int, float, bool)):
            sys.exit(f"check: env.{k} must be a string (or number/bool)")
    print(f"OK: {cfg_path} ({len(cfg.get('dotfiles') or {})} dotfiles, "
          f"{sum(len(v) for v in pkgs.values() if isinstance(v, list))} pkgs, "
